...
이 프로젝트의 목적은 서비스 자체보다 **AI와 함께 문제를 정의하고 개발하는 과정**에 있습니다. 서버/인프라(NestJS, Prisma, Docker)는 이 과정에서 자연스럽게 익힌 덤입니다. 아래는 실제로 밟은 흐름과 레포에 남은 근거입니다.

### 1. 문제 정의 — 요구사항 단계적 확정

- `requirements/`에 요구사항 문서를 버전별로 남깁니다. 1차 문서([`_20260723`](./requirements/3m-log%20요구사항_20260723.md))로 MVP를 구현한 뒤, 써보면서 드러난 문제를 2차 문서([`_20260728`](./requirements/3m-log%20요구사항_20260728.md))에 반영했습니다: 게스트 이용, 마이페이지, 회원탈퇴 추가, 그리고 **인기 태그 집계 기준을 "전체 사용자" → "본인 기준"으로 재변경**(1차 문서에서 내린 결정을 스스로 뒤집은 사례입니다).
- AI에게 요구사항 전체를 매번 새로 던지지 않고, 기존 문서를 베이스로 "이번에 추가/변경되는 것만" 정리하게 해서 변경 이력과 변경 이유가 diff로 남게 했습니다.

### 2. 설계 — 구현 전 도메인 모델 확정

- [`docs/domain-model.md`](./docs/domain-model.md): 애그리거트 경계(예: Tag를 별도 애그리거트로 두지 않고 정규화 저장만 하는 이유)를 코드 작성 전에 AI와 논의해서 먼저 문서로 정리했습니다.
- 인기 태그 집계 기준, 검색 대상 범위처럼 애매하게 넘어가기 쉬운 정책은 문서 안에 "확인된 사항"으로 못박아, 이후 세션에서 같은 질문이 반복되지 않게 했습니다.

### 3. 작업 분해 & 추적

- [`requirements/tasks.md`](./requirements/tasks.md): 요구사항 문서별로 체크리스트를 관리합니다. 새 요구사항 문서가 추가되면 그 문서 기준 섹션도 추가되어, "지금 뭐가 끝났고 뭐가 남았는지"를 사람과 AI가 같은 파일로 확인합니다.

### 4. 구현 — 세션 간 지식 유지

- [`CLAUDE.md`](./CLAUDE.md)에 "재발 가능성 높은 함정"을 세션마다 누적합니다 (Node 버전 문제, Prisma 7 드라이버 어댑터, Passport 전략 초기화 이슈 등). 다음 세션의 AI가 같은 삽질을 반복하지 않게 하는 장치입니다.
- 세션이 끝날 때 그 세션에서 얻은 교훈을 CLAUDE.md에 반영하는 것을 습관화했습니다.

### 5. 검증

- 기능 구현과 테스트(유닛 + e2e)를 같이 요구해서, "동작하는 것처럼 보임"이 아니라 테스트 통과와 체크리스트로 완료를 판단했습니다.

### AI에게 맡긴 것 vs 직접 판단한 것

- **직접 판단**: 요구사항 우선순위, 도메인 경계 확정, 정책 변경(인기 태그 집계 기준을 되돌린 것)처럼 정답이 없는 결정입니다.
- **AI에게 맡긴 것**: NestJS/Prisma/Next.js처럼 처음 다루는 스택의 구현 디테일, 보일러플레이트 작성, 에러 원인 분석입니다.
